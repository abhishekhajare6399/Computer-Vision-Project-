...
def single_face(img):
    ...
    # Find the minimum bounding rectangle around all detected faces
    if len(faces) > 0:
        x, y, w, h = cv2.boundingRect(np.vstack(faces))

        # Draw a rectangle over the merged bounding box of all faces
        cv2.rectangle(img_array, (x, y), (x + w, y + h), (255, 0, 0), 2)

        # Draw rectangles around eyes in the merged face region
...
def main():
    ...
    summary_choice = st.selectbox("Select Activity", ["Computer Vision Operation On Image"])
